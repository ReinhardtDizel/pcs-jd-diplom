...
| `search.PageEntry`   | Класс, описывающий один элемент результата одного поиска. Он состоит из имени пдф-файла, номера страницы и количества раз, которое встретилось это слово на ней        |
| `search.SearchEngine`   | Интерфейс, описывающий поисковый движок. Всё что должен уметь делать поисковый движок, это на запрос из слова отвечать списком элементов результата ответа        |
| `search.BooleanSearchEngine`   | Реализация поискового движка, которую вам предстоит написать. Слово `Boolean` пришло из теории информационного поиска, тк наш движок будет искать в тексте ровно то слово, которое было указано, без использования синонимов и прочих приёмов нечётного поиска        |
...
Также, списки ответов для каждого слова должны быть отсортированы в порядке уменьшения поля `count`. Для этого предлагается классу `search.PageEntry` сразу реализовывать интерфейс `Comparable`.
...
```
[search.PageEntry{pdf=Этапы оценки проекта_ понятия, методы и полезные инструменты.pdf, page=12, count=6}, search.PageEntry{pdf=Этапы оценки проекта_ понятия, методы и полезные инструменты.pdf, page=4, count=3}, search.PageEntry{pdf=Этапы оценки проекта_ понятия, методы и полезные инструменты.pdf, page=5, count=3}, search.PageEntry{pdf=1. DevOps_MLops.pdf, page=5, count=2}, search.PageEntry{pdf=Что такое блокчейн.pdf, page=1, count=2}, search.PageEntry{pdf=Что такое блокчейн.pdf, page=3, count=2}, search.PageEntry{pdf=Этапы оценки проекта_ понятия, методы и полезные инструменты.pdf, page=2, count=1}, search.PageEntry{pdf=Этапы оценки проекта_ понятия, методы и полезные инструменты.pdf, page=11, count=1}, search.PageEntry{pdf=1. DevOps_MLops.pdf, page=3, count=1}, search.PageEntry{pdf=1. DevOps_MLops.pdf, page=4, count=1}, search.PageEntry{pdf=Что такое блокчейн.pdf, page=2, count=1}, search.PageEntry{pdf=Что такое блокчейн.pdf, page=4, count=1}, search.PageEntry{pdf=Что такое блокчейн.pdf, page=5, count=1}, search.PageEntry{pdf=Что такое блокчейн.pdf, page=7, count=1}, search.PageEntry{pdf=Что такое блокчейн.pdf, page=9, count=1}, search.PageEntry{pdf=Продвижение игр.pdf, page=7, count=1}, search.PageEntry{pdf=Как управлять рисками IT-проекта.pdf, page=2, count=1}]
```
